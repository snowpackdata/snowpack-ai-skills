...
generated entries, so anything downstream (a notes upload, a standup, a human skim) has one
place to look.
...
Only when `integrations.notes_api.enabled` is `true` in `capabilities.yml`:

```bash
<integrations.notes_api.script> upload time-log <slug> ~/.local/share/time-logger/raw/combined/time-log_YYYY-MM-DD_<slug>.md YYYY-MM-DD
```

This upserts — re-running with the same date overwrites in place. Report the key the script
prints. If the script is missing or fails, say so and continue; the upload is never a blocker.

## Sync the dashboard

After building (and uploading, if enabled), refresh the store so the Slack/Calendar panels and
the per-day upload chips reflect the new data:

```bash
node ~/.local/share/time-logger/app/dashboard/scripts/fetch-uploads.mjs
