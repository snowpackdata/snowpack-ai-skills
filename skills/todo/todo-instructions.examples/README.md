...
| `TEMPLATE.md` | Something else — Linear, a private notes API, anything that counts as *tracking* a todo somewhere other than the file itself |
...
2. **List** and the optional **Discover** are read-only — they report current status (List) or
   ...
   `/todo list` is what turns a Discover result into a new item, not the instructions file.
...
4. Every operation returns (or works from) an id in the form `<backend>:<identifier>` — e.g.
   `gh:owner/repo#123` — so the item's `id` field stays the single place `/todo done <id>` and
